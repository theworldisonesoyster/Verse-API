...
| Channel | log_channel | 打印消息时会把通道类名作为前缀，例如「[log_channel]: #Message」。 |
| DefaultLevel | log_level | 设置显示消息的默认日志级别。级别详情见 log_level 枚举；默认 log_level.Normal。 |

### Functions
| Function Name | Description |
| Print | 以给定日志级别打印 Message。 |
| Print | 以给定日志级别打印 Message 诊断。 |
| PrintCallStack | 以给定日志级别打印当前脚本调用栈。 |
