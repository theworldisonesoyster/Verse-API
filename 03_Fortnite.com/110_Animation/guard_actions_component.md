...
| MovementSpeedMultiplier | ?float | 移动速度倍率（钳制在 0.5~2 之间）。 |
| TickEvents | ?tick_events | 设置 TickEvents.PrePhysics 与 TickEvents.PostPhysics 回调，在对象物理更新前/后接收逐帧更新。 |

### Functions
| Function Name | Description |
| Attack | 攻击目标；目标必须已被侦测。 |
| Crouch | 切换为蹲伏姿态；不中断则永不完成。 |
| Focus | 看向指定位置。LockFocus 为 false 时，NPC 面向该位置后动作即停止。 |
| Focus | 看向指定实体。LockFocus 为 false 时，NPC 面向该实体后动作即停止。 |
...
| Jump | 触发一次跳跃。 |
| MoveInRangeToAttack | 移动到可攻击当前目标的范围内。 |
| NavigateTo | 向指定的导航目标移动。 |
...
| PlayRandomEmote | 从角色表情库中随机播放一个表情。 |
| RemoveFromEntity | 把组件从实体上移除。被移除的组件会离开场景，且之后只能加回同一个实体。流程经过 OnEndSimulation → OnRemovingFromScene。 |
| Revive | 复活指定目标。 |
| RoamAround | 在当前位置附近游荡。可用 Tether 指定半径；否则 Guard 会随处游荡。 |
| SendDown | 向此组件发送场景事件，触发 OnReceive。有参与者消费该事件则返回 true。 |
| Slide | 触发一次滑铲。 |
| StandUp | 回到站立状态。 |
| StopNavigation | 停止导航。 |
| Tether | 把 NPC 拴定到某个位置。Radius 单位为厘米。 |
| Tether | 把 NPC 拴定到某个实体。Radius 单位为厘米。 |
| Untether | 解除 NPC 的拴定。 |
