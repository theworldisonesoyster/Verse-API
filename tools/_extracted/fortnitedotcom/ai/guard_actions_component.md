...
| Attack | Attack the target. The target must have been detected. |
| Crouch | Change stance to crouch. Will never complete unless interrupted. |
| Focus | Look at the specified location. If LockFocus is false, the action stops once the NPC is facing the position. |
| Focus | Look at the specified Entity. If LockFocus is false, the action stops once the NPC is facing the entity. |
| GetCurrentDestination | Return the current destination of the character. |
| Idle | Stay idle for a specific duration. |
...
| Jump | Trigger a jump. |
| MoveInRangeToAttack | Move in range to attack the current target. |
| NavigateTo | Navigate toward the specified navigation target. |
...
| PlayRandomEmote | Play a random emote from the character emotes bank. |
...
| Revive | Revive the specified target. |
| RoamAround | Roam around the current position. Use 'Tether' to specify the radius; otherwise, the Fortnite guard will roam anywhere. |
| SendDown | Send a scene event to this component, invoking OnReceive. Returns true if any participant consumed the event. |
| Slide | Trigger a slide. |
| StandUp | Go back to standing. |
| StopNavigation | Stop navigation. |
| Tether | Tether the NPC to a position. 'Radius' is in centimeters. |
| Tether | Tether the NPC to an entity. 'Radius' is in centimeters. |
| Untether | Untether the NPC. |
