...
| MaxStackSize | ??int | The maximum amount this component can hold. If unset, it holds an unlimited amount. |
| split_prefab_type | concrete_subtype(castable_subtype(entity)) | The prefab used when this entity merges with another or when it is split into a new instance. |
| StackSize | ?int | The current amount of this entity held in the stack. |
...
| CanMergeInto | Succeeds if this entity can be merged into the target entity. Merging an entity with itself will always fail. |
| CanMergeInto | Succeeds if this entity can be merged into the target entity. Merging an entity with itself will always fail. Note that merging should be checked in both directions! |
...
| MergeInto | Attempts to merge this entity into the specified entity. Fails if entities cannot be merged. If TargetAmount is specified, only that amount will try to be merged into this entity. By default, the entire stack will try to merge. If the specified amount is invalid, the merge will fail. |
...
| SetMaxStackSize | Sets the maximum stack size for this component. If NewMaxStackSize is false, stack size is unlimited. If ClampStackSize is true, StackSize will be clamped to NewMaxStackSize. |
| SetStackSize | Sets the stack size of this component. If the provided stack size is invalid, e.g. negative or exceeding MaxStackSize, stack size will not change. |
| Split |  |
| Split |  |
